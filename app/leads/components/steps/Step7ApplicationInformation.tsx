...
export default function ApplicationInfoTab({
  ...
  const [currentView, setCurrentView] = useState<"applying" | "receiving">(
    "applying",
  );
  const [selectedRole, setSelectedRole] = useState<"donor" | "attorney">(
    "donor",
  );
    ...
    const init = async () => {
      ...
      try {
        ...
        const attorneysRes = await fetch(
          `/api/lpa-document-attorneys?lpaDocId=${activeLpaId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        const notifyRes = await fetch(
          `/api/people-to-notify?lpaDocId=${activeLpaId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        const applicantRes = await fetch(
          `/api/lpa-document-applicants?lpaDocId=${activeLpaId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const attorneyId =
        selectedRole === "attorney" ? selectedAttorneyId : null;
      ...
      const registeringName =
        currentDisplayApplicant?.name || donorRecord?.name || "Donor";

      let receivingName = "Donor";
      if (selectedRecipientType === "donor")
        receivingName = donorRecord?.name || "Donor";
      else if (selectedRecipientType === "attorney")
        receivingName =
          attorneyRecords.find((a) => a.id === selectedRecipientId)?.name ||
          "Attorney";
      else if (selectedRecipientType === "other")
        receivingName =
          otherPeople.find((p) => p.id === selectedRecipientId)?.name ||
          "Other";
      ...
      <div className="max-w-3xl w-full">
        {currentView === "applying" ? (
          <>
            <h4 className={styles.stepHeading}>
              Who is{" "}
              <span className="text-cyan-400">applying to register?</span>
            </h4>

            <div className={styles.dividerZenco}></div>

            <p className={"font-medium text-black text-lg mb-3"}>
              This document can&apos;t be used until it is registered by the
              Office of the Public Guardian (OPG).
            </p>
            <p className={"font-medium text-black text-lg mb-3"}>
              {" "}
              Only the donor (
              <span className="text-[#08b9ed]">{donorRecord?.name}</span>) or
              one of the attorneys can apply to register this document.
            </p>
            <p className={"font-medium text-black text-lg mb-3"}>
              {" "}
              Select from the option below whether the donor (
              <span className="text-[#08b9ed]">{donorRecord?.name}</span>) is
            ...
            <div className="mt-6">
              ...
              <select
                ...
                className="w-full border border-[#26b050] rounded-sm p-3 bg-white outline-none transition-shadow duration-200 focus:ring-2 focus:ring-gray-300"
              >
            ...
            {selectedRole === "attorney" && attorneyRecords.length === 1 && (
              <p className="text-sm text-gray-500 mt-4 mb-2">
                You only have one attorney, they have been automatically
                selected.
              </p>
            )}
            ...
            {selectedRole === "attorney" && attorneyRecords.length === 0 && (
              <Alert severity="warning" className="mt-4">
                No primary attorneys found. Please go back to Step 4 and add at
                least one primary attorney if you wish to select an attorney as
                the applicant.
              </Alert>
            )}
          </>
          ...
          <>
            <h4 className={styles.stepHeading}>
              Who should{" "}
              <span className="text-cyan-400">receive the document?</span>
            </h4>

            <div className={styles.dividerZenco}></div>

            <p className={styles.pZenco + " font-normal text-gray-600"}>
              Once this document is registered with the Office of the Public
              Guardian (OPG) it will be sent to the person listed below.
            </p>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value === "donor")
                    setSelectedRecipientId(donorRecord?.id || "");
                  if (
                    e.target.value === "attorney" &&
                    attorneyRecords.length === 1
                  )
                    setSelectedRecipientId(attorneyRecords[0].id);
                }}
                className={styles.applicantDropdown}
              >
            ...
            <div className="mt-8 space-y-4">
              ...
              {selectedRecipientType === "donor" && donorRecord && (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center ${selectedRecipientId === donorRecord.id ? "bg-gray-400" : "bg-gray-100"}`}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-6 w-6 ${selectedRecipientId === donorRecord.id ? "text-white" : "text-gray-400"}`}
                        viewBox="0 0 20 20"
                        fill="currentColor"
                      >
                        <path
                    ...
                    <div>
                      ...
                      <p
                        className={`${selectedRecipientId === donorRecord.id ? "text-gray-300" : "text-gray-500"}`}
                      >
                        {donorRecord.email}
                      </p>
                  ...
                  <input
                    type="checkbox"
                    checked={selectedRecipientId === donorRecord.id}
                    readOnly
              ...
              {selectedRecipientType === "attorney" &&
                attorneyRecords.map((person) => (
                  <div
                    key={person.id}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center ${selectedRecipientId === person.id ? "bg-gray-400" : "bg-gray-100"}`}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className={`h-6 w-6 ${selectedRecipientId === person.id ? "text-white" : "text-gray-400"}`}
                          viewBox="0 0 20 20"
                          fill="currentColor"
                        >
                          <path
                      ...
                      <div>
                        ...
                        <p
                          className={`${selectedRecipientId === person.id ? "text-gray-300" : "text-gray-500"}`}
                        >
                          {person.email}
                        </p>
        ...
        <div className="flex items-center justify-between mt-12 mb-10">
          ...
          <button
            ...
            className={`p-4 rounded text-white font-bold text-lg transition-all flex items-center justify-center min-w-45 cursor-pointer ${
              currentView === "applying"
                ? "bg-[#08b9ed] hover:bg-cyan-600"
                : "bg-[#08b9ed] hover:bg-cyan-600"
            }`}
          >
            {isSubmitting ? (
